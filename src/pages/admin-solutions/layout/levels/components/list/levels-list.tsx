...
import { type SolutionLevelElement } from 'src/types/solutions-types'

export const SolLevelsist: FC = () => {
	const filterValues = useAppSelector(getFiltrationValues)
	const [currentPage, setCurrentPage] = useState(1)
	const [itemsPerPage, setItemsPerPage] = useState<number | 'all'>(100)

	const { data } = useGetAllSolLevelsQuery({
		title: filterValues.title,
		limit: itemsPerPage === 'all' ? undefined : itemsPerPage,
		page: itemsPerPage === 'all' ? undefined : currentPage,
	})
	const { refetch: getNewId } = useGetNewIdSolLevelQuery(null)
	const [deleteTypeById] = useDeleteSolLevelByIdMutation()
	...
	const formatObjectsTableData = (data: SolutionLevelElement[]) => {
		return data.map((el) => {
			return {
				rowId: el.id,
				cells: [
